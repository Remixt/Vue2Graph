...
class Group:
    def __init__(self, group_id, parent_graph, label=None, shape="rectangle",
        ...
        if shape not in node_shapes:
            raise RuntimeWarning("Node shape %s not recognized" % shape)
        self.shape = shape
        ...
        if font_style not in font_styles:
            raise RuntimeWarning("Font style %s not recognized" % font_style)
        ...
        if edge_type not in line_types:
            raise RuntimeWarning("Edge type %s not recognized" % edge_type)
...
class Edge:
    def __init__(self, node1, node2, label="", arrowhead="standard", arrowfoot="none",
        ...
        if arrowhead not in arrow_types:
            raise RuntimeWarning("Arrowhead type %s not recognized" % arrowhead)

        self.arrowhead = arrowhead

        if arrowfoot not in arrow_types:
            raise RuntimeWarning("Arrowhead type %s not recognized" % arrowfoot)

        self.arrowfoot = arrowfoot

        if line_type not in line_types:
            raise RuntimeWarning("Edge type %s not recognized" % line_type)
